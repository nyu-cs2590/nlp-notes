...
\begin{frame}
    {Applications}
    \header{Text generation}: MT, summarization, chit-chat dialogue, image caption, story generation etc.

    \header{Structured prediction}:
    \vspace{-1em}
    \begin{figure}
...
\begin{frame}
    ...
    \header{Task}: given the reference(s) of each source sentence, evaluate the quality of the generated sequences.

    \header{Main idea}: good generations should have high overlap with the reference.
\end{frame}
...
\begin{frame}
    ...
    \header{candidate length} $C=\sum_{(x,c,r)} \text{len}(c)$

    \header{reference length} $R=\sum_{(x,c,r)} \argmin_{a\in\pc{\text{len}(r_1),\ldots,\text{len}(r_k)}} |a-\text{len}(c)|$\\
    \begin{itemize}
